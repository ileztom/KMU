namespace KMU
{
    internal class Program
    {
        static void Main(string[] args)
        {
            ...
            double result = CombinedMethodSolver(equation, derivative, initialGuess, epsilon);

            Console.WriteLine($"Решение: x = {result}");
            Console.WriteLine($"Значение уравнения в найденной точке: y = {equation(result)}");
        }

        static double CombinedMethodSolver(Func<double, double> equation, Func<double, double> derivative, double initialGuess, double epsilon)
        {
            ...
            {
                x0 = x1;
                x1 = x0 - lambda * equation(x0) / derivative(x0);
            }
